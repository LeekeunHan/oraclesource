...
-- [문제2] employees 테이블의 first_name , last_name , job_id조회
SELECT
    first_name,
    last_name,
    job_id
...
-- 위 세개의 열은 별칭을 붙여서 조회 
-- 조회할때 부서번호를 기준으로 내림차순으로 정렬하되 부서번호가 같다면 
...
-- [문제4] 사원번호가 176인 사원의 last_name과 department_id 조회 
SELECT
    last_name,
    department_id
FROM
...
-- [문제8] 커미션을 받는 모든 사원들의 last_name,salary,commission_pct 를 조회한다
-- 연봉의 내림차순 , commission_pct 의 내림차순 정렬 
SELECT
    last_name,
    salary,
    commission_pct
...
-- [문제9] 연봉이 2500 , 3500 , 7000 이 아니면 , 직업이 SA_REP , ST_CLERK인 사원조회 
-- 전체 정보 조회 
SELECT
    *
FROM
...
-- LIKE 와 와일드카드 사용 
-- [문제 12] '2004' 년도에 고용된 모든 사람들의 last_name,hire_date를 조회하여 
-- 입사일 기준으로 오름차순 정렬 
SELECT
    last_name,
    hire_date
...
-- [문제15] last_name 에 a 혹은 e가 들어있는 사원들의 last_name 조회후
-- last_name 오름차순 출력
SELECT
    last_name
FROM
...
-- [문제16] last_name 에 a 와 e가 들어있는 사원들의 last_name 조회후
-- last_name 오름차순 출력
SELECT
    last_name
FROM
...
-- [문제17] manager_id 없는 사원들의 정보를 조회 
SELECT
    last_name,
    job_id,
    manager_id
...
--[문제] ST_CLERK인 JOB_ID 를 가진 사원이 없는 부서 ID조회 
...
--[문제] commission_pct 가 널이 아닌 사원들중에서 commission = salary * commission_pct를 
...
-- [문제] first_name 이 Curtis 인 사람의 first_name , last_name , phone_number, job_id조회
-- 단 , job_id 의 결과는 소문자로 출력하기
SELECT
    first_name,
    last_name,
    phone_number,
...
-- job_id 조회하기. 단 job_id 가 IT_PROG인 사원의 경우 프로그래머로 변경하여 출력 
...
-- [문제] job_id 가 AD_PRES, PU_CLERK 인 사원들의  employee_id , first_name , last_name , department_id,
-- job_id 조회하기, 단 사원명은 first_name 과 last_name을 연결하여 출력 
...
-- [문제] 부서id 가 80 인 사원에 대해서 서로 다른 세율 적용
-- 2000 미만 - 0% 
-- 4000 미만 - 9%
-- 6000 미만 - 20%
-- 8000 미만 - 30%
...
-- [실습] 자신의 담당 매니저의 고용일보다 빠른 입사자 찾기(employees셀프조인) 
SELECT
    e1.employee_id,
...
-- [실습] 도시이름이 T로 시작하는 지역에 사는 사원들의 사번 , last_name, department_id , 
...
-- [실습] 위치 ID 가 1700 인 사원들의 employee_id , last_name, department_id, salary
...
-- [실습] 각 부서별 평균 연봉(소수점2자리까지) , 사원 수 조회 
...
-- [실습] 기존의 작업을 여전히 가지고있는 사원들의 employee_id , job_id 출력 
